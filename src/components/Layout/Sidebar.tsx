...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, isMinimized, onToggleMinimize }) => {
  ...
  const getMenuItems = () => {
    ...
    if (user?.role === 'teacher') {
      menuItems.push(
        { id: 'students', icon: Users, label: t('sidebar.students'), path: '/students' },
      ...
      menuItems.push(
        { id: 'users', icon: Users, label: t('sidebar.teachersAndStudents'), path: '/users' },
        { id: 'reports', icon: BarChart3, label: t('sidebar.reports'), path: '/reports' }
      );
